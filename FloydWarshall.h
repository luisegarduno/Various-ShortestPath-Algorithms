...
public:
    ...
    void findShortestPath(Graph<Vertex>&, int);
...
template<class W>
void FloydWarshall<W>::findShortestPath(Graph<Vertex>& thisGraph, int numOfNodes){
    Graph<Vertex> graphs;
    for(int sourceNode = 1; sourceNode < numOfNodes; sourceNode++) {
        ...
        while (!magic_nodes.empty()) {
            ...
            for (int it = 1; it <= n; it++) {
                ...
                //cout << vertexA << " " << vertexB << " " << edge_weight << endl;
